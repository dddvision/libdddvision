...
void trackFeatures(double*, double*, double*, double, double,
		        		  ...
		        		  int nrhs, const mxArray *prhs[] )
{
  ...
  if( (mxGetM(prhs[10])!=1) || (mxGetN(prhs[10])!=1) || (mxGetM(prhs[11])!=1) || (mxGetN(prhs[11])!=1) || (mxGetM(prhs[12])!=1) || (mxGetN(prhs[12])!=1))
  {  
  ...
  for(k=0;k<K;k++)
  {
    ...
    trackFeatures(Ia,gxa,gya,xa[k]-1.0,ya[k]-1.0,Ib,gxb,gyb,xbe[k]-1.0,ybe[k]-1.0,L,&xb[k],&yb[k],m,n,(int)floor(win+0.5),RESIDUE_THRESH);
...
/*** tracking algorithm ***/
void trackFeatures(double *Ia, double *gxa, double *gya, double xa, double ya,
                   double *Ib, double *gxb, double *gyb, double xbe, double ybe, double L,
                   double *xb, double *yb, int m, int n, int win, double RESIDUE_THRESH)
{
  int win2=win*win;
  int mmwin=m-win;
  int halfwin=win/2;
